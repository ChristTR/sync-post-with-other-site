...
function spmv2_log($msg) {
    ...
    @file_put_contents(SPMV2_LOG_FILE,
...
add_action('admin_init', function(){
    register_setting('spm_v2_settings_group','spm_v2_settings',[
...
class SPM_v2_Core {
    ...
    public function render_settings_page(){
        ...
        <div class="wrap">
          ...
          <form method="post" action="options.php">
            ...
            <table class="form-table"><tr>
              ...
              <td>
                <label><input type="checkbox" name="spm_v2_settings[auto_sync]" <?php checked($s['auto_sync'],true);?>>
            ...
            <table class="form-table"><tr>
              ...
              <td>
                ...
                <button type="button" class="button button-small" onclick="genNew()"><?php spmv2_e('Generate New');?></button>
              </td>
            </tr><tr>
              <th><?php spmv2_e('Force HTTPS');?></th>
              <td><label><input type="checkbox" name="spm_v2_settings[security][force_ssl]" <?php checked($s['security']['force_ssl'],true);?>>
              <?php spmv2_e('Force HTTPS');?></label></td>
            </tr></table>
            <?php submit_button(); ?>
          </form>
        </div>
        ...
        function genNew(){
          ...
          let r=Array.from(crypto.getRandomValues(new Uint8Array(32)))
    ...
    public function render_meta_box($post){
        try {
            ...
            $n    = wp_create_nonce('spm_v2_manual_sync'); ?>
    ...
    public function handle_manual_sync(){
        ...
        spmv2_log("manual_sync: post{$pid},auto=".($disabled?'0':'1'));
    ...
    public function handle_post_save($id,$post,$u){
        spmv2_log("save_post: id{$id},status={$post->post_status},upd={$u}");
    ...
    private function process_job($job){
        spmv2_log("process_job: #{$job['id']} post{$job['post_id']}");
        global $wpdb;
        try {
    ...
    private function sync_post($post,$host){
        ...
        $pl = ['post'=>$this->prepare_post_data($post),
               'meta'=>$this->prepare_post_meta($post->ID)];
        spmv2_log("sync_post to {$host['url']}:".wp_json_encode($pl));
        $resp=wp_remote_post(trailingslashit($host['url']).'wp-json/spm/v2/sync',[
            ...
            'body'=>wp_json_encode($pl),'timeout'=>30
        ]);
    ...
    private function record_sync_success($id,$host){
        spmv2_log("success post{$id}@{$host}");
    ...
    private function handle_sync_error($job,$err){
        spmv2_log("error job{$job['id']}:".$err);
        global $wpdb;
        $a=$job['attempts']+1;
        $n=time()+($this->base_delay*pow(2,$a));
        $wpdb->update($this->queue_table,[
            'attempts'=>$a,
            'last_attempt'=>current_time('mysql'),
            'next_attempt'=>date('Y-m-d H:i:s',$n)
        ],['id'=>$job['id']]);
    }
    ...
    public function handle_media_request($req){
        $raw=$req->get_body();
        $hdr=$req->get_header('Content-Disposition');
        if(preg_match('/filename="([^"]+)"/',$hdr,$m)) $fn=sanitize_file_name($m[1]);
        else $fn='upload-'.time();
    ...
    // --- Utils ---
    private function generate_jwt($s){
        $h=base64Url(json_encode(['alg'=>'HS256','typ'=>'JWT']));
        $p=base64Url(json_encode(['iss'=>get_site_url(),'iat'=>time(),'exp'=>time()+300]));
        return "$h.$p.".base64Url(hash_hmac('sha256',"$h.$p",$s,true));
    }
    private function generate_signature($post,$s){
        return hash_hmac('sha256',$post->ID.'|'.$post->post_modified,$s);
    }
    private function validate_jwt($t,$s){
        list($h,$p,$sig)=explode('.',$t);
        $v=base64Url(hash_hmac('sha256',"$h.$p",$s,true));
        $pl=json_decode(base64_decode(strtr($p,'-_','+/')),true);
        return hash_equals($sig,$v)&&(!empty($pl['exp'])&&$pl['exp']>time());  
    }
    private function validate_signature($sig,$d,$s){
        return hash_equals(hash_hmac('sha256',$d,$s),$sig);
    }
    private function get_host_config($u){
        foreach($this->settings['hosts']as$h) if($h['url']===$u) return $h;
        return null;
    }
    ...
    private function prepare_post_meta($id){
        $o=[];foreach(get_post_meta($id)as$k=>$v){
            if(strpos($k,'_spm_v2_')===0) continue;
            $o[$k]=array_map('maybe_unserialize',$v);
        }return $o;
    }
    private function update_post_meta($id,$m){
        foreach($m as$k=>$v){
            if(strpos($k,'_spm_v2_')===0) continue;
            delete_post_meta($id,$k);
            foreach((array)$v as$x)add_post_meta($id,$k,$x);
        }
    }
    private function get_author_data($aid){
        $u=get_userdata($aid);
        return $u?['display_name'=>$u->display_name,'user_email'=>$u->user_email,'user_login'=>$u->user_login]:[];
    }
}
